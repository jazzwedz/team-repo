...
// Document output structure editor — one page for every document kind
// (/doc-structure/dsd, /doc-structure/fs, …).
//
// The counterpart to the Agents page: agents are HOW each section is
// written; this defines WHAT chapters the document has — their titles, the
// guidance (what each must contain), which writer owns them, and the
// critics' focus. Saved to the kind's structure file in the data repo and
// read by the generator. The kind's writer agents and critics are fixed
// (they map to trainable personas); everything else here is editable.
...
import { ListTree, Loader2, AlertCircle, Plus, Trash2, ChevronUp, ChevronDown, Save, RotateCcw, Info } from "lucide-react"
import { DOC_KINDS, isDocKind } from "@/lib/doc-kinds"
...
export default function DocStructurePage() {
  ...
  async function save() {
    ...
    setError(null)
    try {
      const r = await fetch(`/api/doc-structure/${kind}`, {
        method: "PUT",
      ...
      setSha(d.sha)
      setDirty(false)
      setSavedMsg(`Saved — applies to the next ${def!.short} generation.`)
    } catch (e) {
  ...
  async function resetDefaults() {
    if (!confirm(`Reset the ${def!.short} output structure to the built-in default? Your customisations will be removed.`)) return
    setResetting(true)
    setError(null)
    try {
      const r = await fetch(`/api/doc-structure/${kind}/reset`, { method: "POST" })
      const d = await r.json()
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            ...
            {def.short} Output Structure
          </h1>
          <p className="text-muted-foreground mt-1">
            {loading
              ? "Loading…"
              : `${def.label} — ${totalChapters} chapters across ${structure?.groups.length ?? 0} writers.`}
          </p>
        </div>
