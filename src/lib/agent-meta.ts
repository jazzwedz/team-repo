...
export const AGENT_USAGE: Record<string, string> = {
  ...
  "fs-writer-purpose": "FS generation (agent team) → writes the Introduction (demand, objective, business overview), the Scope and the Actors, Stakeholders & Glossary.",
  "fs-writer-functional": "FS generation → writes the Use Cases (UC-NN): use-case tables, process flows and the per-use-case management-rules tables (RG-NN).",
  "fs-writer-data-rules": "FS generation → writes the Management Rules & Calculations register (RG-NN), Data Requirements, and Interfaces, Batches & Notifications.",
  "fs-writer-quality": "FS generation → writes Non-Functional Requirements (NFR-NN), Acceptance Criteria (AC-NN), the Decisions & Open Points register and the Appendix.",
  "fs-critic-grounding": "FS generation → flags inventions/contradictions vs the verified facts.",
  "fs-critic-completeness": "FS generation → flags empty template sections, processes without a use case, rules without an RG entry and use cases without acceptance criteria.",
  "fs-critic-clarity": "FS generation → flags untestable statements, options left open as if decided, and rules stated twice in different words.",
  "fs-critic-consistency": "FS generation → checks UC / RG / NFR / AC ids are unique and identical across chapters.",
